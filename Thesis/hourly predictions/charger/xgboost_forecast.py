...
df["weekday"]        = df["ds"].dt.weekday          # 0 = maandag … 6 = zondag
...
# ───── NEW 3a  EXPLICIETE VRIJDAG & PRE-HOLIDAY DUMMIES ──────────
df["is_friday"]       = (df["weekday"] == 4).astype(int)
df["is_pre_holiday"]  = (
    (df["ds"].dt.normalize() + pd.Timedelta(days=1))
    .isin(be_holidays)
).astype(int)
...
for day in days_iter:
    ...
    # voeg dezelfde helper-features toe aan de dagset
...
r2_by_wd = {day_map[d]: r2_score(
    test_bt[test_bt['weekday']==d]['y'],
    test_bt[test_bt['weekday']==d]['yhat'])
    for d in range(5)}
...
# ══════════════════════ 7. PLOTS ════════════════════════════════
# Figuur 23 – Error-heatmap (absolute fout per uur × weekdag)
heatmap_data = (test_bt.pivot_table(index='weekday', columns='hour',
...
plt.xlabel("Date")
plt.ylabel("Charger consumption in kWh")
plt.legend()
plt.tight_layout()
plt.show()

# Figuur 24 – Predicted vs Actual scatter (uur-resolutie)
...
# ───────────── 6a  ABSOLUTE-ERROR PER WEEKDAY ──────────────
day_map      = {0:'Mon', 1:'Tue', 2:'Wed', 3:'Thu', 4:'Fri', 5:'Sat', 6:'Sun'}
mae_by_wd    = (test_bt
                .groupby("weekday")["abs_err"]
                .mean()
                .rename(index=day_map)
                .round(2))

print("\n--- Mean Absolute Error by Weekday ---")
for d, v in mae_by_wd.items():
    print(f"{d}: {v:.2f} kWh")

# (optional) quick bar-plot.
plt.figure(figsize=(12,5))
plt.bar(mae_by_wd.index, mae_by_wd.values)
plt.ylabel("MAE (kWh)")
plt.title("Mean Absolute Error per Weekday")
for x, v in zip(mae_by_wd.index, mae_by_wd.values):
    plt.text(x, v + 0.02, f"{v:.2f}", ha='center', va='bottom')
plt.tight_layout(); plt.show()
