...
TOTAL_HEADCOUNT = 105
SESSION_KWH      = 9.5
TIMESTEPS        = 168   # use one week of history per sample
...
for day in days_iter:
    ...
    X_seq, y_seq = [], []
    feat_vals = train[FEATURES].values
    y_vals    = train["y"].values
    for i in range(len(feat_vals) - TIMESTEPS):
        X_seq.append(feat_vals[i:i+TIMESTEPS])
        y_seq.append(y_vals[i+TIMESTEPS])
    X_train = np.array(X_seq)
    y_train = np.array(y_seq)

    # build & train LSTM
    model = Sequential([
        LSTM(50, input_shape=(TIMESTEPS, len(FEATURES))),
        Dense(1)
    ])
    model.compile(optimizer=Adam(1e-3), loss='mae')
    model.fit(X_train, y_train, epochs=5, batch_size=32, verbose=0)

    # build 24-row feature frame for target day
    ...
    # iterative forecast
    full_feats = np.vstack([feat_vals, day_df[FEATURES].values])
    preds = []
    for t in range(len(day_df)):
        idx = len(feat_vals) + t
        seq = full_feats[idx-TIMESTEPS:idx]
        preds.append(model.predict(seq[np.newaxis], verbose=0)[0,0])
    day_df["yhat"] = preds
...
# R² on daily sums
daily = test_bt.groupby(test_bt["ds"].dt.date).agg(y_sum=('y','sum'),
                                                   yhat_sum=('y','sum'))
