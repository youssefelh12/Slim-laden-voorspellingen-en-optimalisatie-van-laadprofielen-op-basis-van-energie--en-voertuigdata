...
# CSV output filename for 15-minute data
CSV_FILENAME = "api_data/15minute_api_data.csv"
...
def get_electricity_consumption(smappee_location_id, start_date, end_date, token, aggregation=7):
    """
    Retrieve electricity consumption data between start_date and end_date.
    
    Parameters:
      - aggregation: 7 for 15-minute values.
...
def extract_consumption_per_phase(consumption_json, channel_lookup):
    ...
    Groups data by 15-minute interval using the format "YYYY-MM-DD HH:MM:00",
    where MM is rounded to the nearest 15-minute boundary.
    """
    ...
    for record in consumption_json["consumptions"]:
        ...
        minute = (record_date.minute // 15) * 15
        interval_key = record_date.replace(minute=minute, second=0, microsecond=0)
        interval_str = interval_key.strftime("%Y-%m-%d %H:%M:%S")
        if interval_str not in interval_consumption:
            interval_consumption[interval_str] = {}
        active_values = record.get("active", [])
        for consumption_index, (meas_name, phase) in channel_lookup.items():
            # Adjust index conversion: if consumption_index is 0, assume it's already 0-based.
            idx = consumption_index if consumption_index == 0 else consumption_index - 1
            if idx < len(active_values):
                value = active_values[idx]
                if value is not None:
                    key = (meas_name, phase)
                    interval_consumption[interval_str][key] = interval_consumption[interval_str].get(key, 0) + value
...
def main():
    ...
    while current_start < now:
        ...
        current_end = current_start + relativedelta(minutes=15)
        if current_end > now:
            current_end = now

        # Fetch consumption data with aggregation=7 (15-minute increments)
        consumption_json = get_electricity_consumption(SMAPPEE_LOCATION_ID, current_start, current_end, token, aggregation=7)
        if consumption_json is None:
            logger.error("Skipping period %s to %s due to API error.", current_start, current_end)
            current_start += relativedelta(minutes=15)
            continue

        interval_data = extract_consumption_per_phase(consumption_json, channel_lookup)
        for interval, data in interval_data.items():
            if interval not in aggregated_data:
                aggregated_data[interval] = {}
            for key, value in data.items():
                aggregated_data[interval][key] = aggregated_data[interval].get(key, 0) + value

        logger.info("Processed data for interval starting %s.", current_start.strftime("%Y-%m-%d %H:%M:%S"))
        current_start += relativedelta(minutes=15)

    # Write the aggregated 15-minute data to CSV.
    with open(CSV_FILENAME, "w", newline="") as csvfile:
        fieldnames = ["Interval", "Measurement", "Phase", "Consumption"]
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        for interval, channels in aggregated_data.items():
