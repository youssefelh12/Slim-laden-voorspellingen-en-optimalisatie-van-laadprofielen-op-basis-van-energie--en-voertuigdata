...
# (Ensure you set these credentials appropriately)
...
# CSV output filename for weekly data
CSV_FILENAME = "weekly_api_data.csv"
...
def get_electricity_consumption(smappee_location_id, start_date, end_date, token, aggregation=3):
    ...
    Parameters:
      - aggregation: Level of detail (use 3 for daily values so that we can aggregate weekly).
...
def extract_consumption_per_phase(consumption_json, channel_lookup):
    ...
    Groups data by week using ISO week format: e.g. '2025-W07'.
    """
    weekly_consumption = {}
    ...
    for record in consumption_json["consumptions"]:
        ...
        if week_str not in weekly_consumption:
            weekly_consumption[week_str] = {}
        ...
        for consumption_index, (meas_name, phase) in channel_lookup.items():
            ...
            if idx < len(active_values):
                ...
                if value is not None:
                    key = (meas_name, phase)
                    weekly_consumption[week_str][key] = weekly_consumption[week_str].get(key, 0) + value
...
def main():
    ...
    while current_start < now:
        # Iterate in weekly intervals.
        current_end = current_start + relativedelta(weeks=1)
        if current_end > now:
            current_end = now

        # Fetch consumption data with aggregation=3 (daily) so we can sum up weekly values.
        ...
        weekly_data = extract_consumption_per_phase(consumption_json, channel_lookup)
        for week, data in weekly_data.items():
            if week not in aggregated_data:
                aggregated_data[week] = {}
            for key, value in data.items():
                aggregated_data[week][key] = aggregated_data[week].get(key, 0) + value

        logger.info("Processed data for week starting %s.", current_start.strftime("%Y-%m-%d"))
        current_start += relativedelta(weeks=1)

    # Write the aggregated weekly data to CSV.
    with open(CSV_FILENAME, "w", newline="") as csvfile:
        fieldnames = ["Week", "Measurement", "Phase", "Consumption"]
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        for week, channels in aggregated_data.items():
