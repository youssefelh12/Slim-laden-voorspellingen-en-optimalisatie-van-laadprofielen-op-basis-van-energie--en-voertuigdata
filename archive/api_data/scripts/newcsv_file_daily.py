...
df = pd.read_csv('./api_data/daily_api_data.csv', parse_dates=['Day'])

# Group by 'Hour' and 'Measurement', summing the 'Consumption' values
grouped = df.groupby(['Day', 'Measurement'])['Consumption'].sum()
...
pivot_df.to_csv('./api_data/aggregated_daily_measurements.csv')
