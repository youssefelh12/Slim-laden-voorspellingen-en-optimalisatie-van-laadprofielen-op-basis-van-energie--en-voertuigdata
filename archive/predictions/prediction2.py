...
# ---------------------------
# Shift the consumption if there are negative values
shift_val = abs(df["Total_consumption"].min()) + 1  # ensures positivity
...
plt.tight_layout()
plt.savefig("results/pred2/correlation_heatmap.png")
plt.close()

# Plot original time series of Total Consumption
...
plt.tight_layout()
plt.savefig("results/pred2/time_series_plot.png")
plt.close()
...
# ---------------------------
# Use the log of consumption as the target
target = "log_consumption"
# Also keep original consumption for error metrics after inversion
...
# Target (log-transformed) and exogenous variables
y = df[target]
exog = df[exog_features]

# Split data into train and test sets using a temporal split
...
# Normalize exogenous features (fit only on training data)
...
# ---------------------------
# Fit SARIMAX Model with a Constant Trend
# ---------------------------
...
try:
    model = SARIMAX(
        ...
        trend='c',  # Include a constant (baseline) trend
    ...
    # Forecast on the log-scale target
...
forecast_df.to_csv("results/pred2/predicted_values_kwh.csv")
print("\nPredicted values have been saved to 'results/pred2/predicted_values_kwh.csv'")
...
feature_importance.to_csv("results/pred2/feature_importance.csv", index=False)
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("results/pred2/forecast_comparison.png")
plt.close()
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("results/pred2/residuals.png")
plt.close()
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("results/pred2/error_distribution.png")
plt.close()

# Remove index name to avoid ambiguity when grouping
forecast_df.index.name = None

# Add additional columns for error analysis
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("results/pred2/error_by_hour.png")
plt.close()
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("results/pred2/error_by_day.png")
plt.close()
