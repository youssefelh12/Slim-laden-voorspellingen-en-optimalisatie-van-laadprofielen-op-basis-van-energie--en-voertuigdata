...
# --- absolute fout per timestamp ---
test_bt['abs_err'] = np.abs(test_bt['y'] - test_bt['yhat'])
# --- absolute percentage error (%), vermijd deling door nul ---
...
# ---------- NIEUW ----------
mae_pct = (mae / (test_bt['y'].mean() + 1e-10)) * 100          # MAE %
p95     = test_bt['abs_err'].quantile(0.95)                    # 95e percentiel
# ---------------------------

print("\n--- Back-test Metrics (Laatste 30 Dagen) ---")
print(f"MAE      : {mae:.2f}")
print(f"MAE %    : {mae_pct:.2f}%")
print(f"p95      : {p95:.2f}")          # ← extra regel
...
# ── Gemiddelde pct_err per weekdag ──
wd_labels     = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']
pct_mae_by_wd = test_bt.groupby('weekday')['pct_err'].mean().reindex(range(7))

# ── Barplot tekenen ──
plt.figure(figsize=(8,4))
bars = plt.bar(wd_labels, pct_mae_by_wd.values, color='seagreen')
plt.ylabel("MAE %")
plt.title("Gemiddelde Absolute Percentage-fout per Weekdag")
