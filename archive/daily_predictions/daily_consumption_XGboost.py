...
# ---------------------------
def prepare_model_data(df, target, exog_features, split_ratio=0.8):
    y = df[target]
    ...
    return y_train, y_test, exog_train_scaled, exog_test_scaled, y_orig_train, y_orig_test

def fit_xgboost_model(y_train, X_train, y_test, X_test):
    # Define an XGBoost regressor with suitable hyperparameters
    ...
    # Predict on the test set (predictions on the log scale)
...
def main():
    ...
    (y_train, y_test, exog_train_scaled, exog_test_scaled, 
     y_orig_train, y_orig_test) = prepare_model_data(df, target, exog_features)
    
    # Train an XGBoost model on the log-transformed target and forecast
    y_pred_log = fit_xgboost_model(y_train, exog_train_scaled, y_test, exog_test_scaled)
    ...
    print("\nAll daily visualizations have been saved in the 'results/daily_xgboost' directory.")
