...
backcharger_cols = ["Chargers achteraan (L1) [kW]", "Chargers achteraan (L2) [kW]", "Chargers achteraan (L3) [kW]"]
grid_cols = ["Grid Organi lbc (L1) [kW]", "Grid Organi lbc (L2) [kW]", "Grid Organi lbc (L3) [kW]"]
solar_cols = [ "Solar (L1) [kW]", "Solar (L2) [kW]", "Solar (L3) [kW]"]

for cols in [charger_cols, backcharger_cols, grid_cols, solar_cols]:
    for col in cols:
        df[col] = df[col].astype(str).str.replace(",", ".").astype(float)

# Compute total charger consumption.
df["total_chargers"] = df["Chargers (L1) [kW]"] + df["Chargers (L2) [kW]"] + df["Chargers (L3) [kW]"]
df["total_backchargers"] = df["Chargers achteraan (L1) [kW]"] + df["Chargers achteraan (L2) [kW]"] + df["Chargers achteraan (L3) [kW]"]
df["total_grid"] = df["Grid Organi lbc (L1) [kW]"] + df["Grid Organi lbc (L2) [kW]"] + df["Grid Organi lbc (L3) [kW]"]
df["total_solar"] = df["Solar (L1) [kW]"] + df["Solar (L2) [kW]"] + df["Solar (L3) [kW]"]
...
# -----------------------------
selected_features = ['total_chargers', 'total_backchargers', 'total_grid', 'total_solar', 'hour_sin', 'hour_cos', 'week_sin', 'week_cos',
...
df['total_backchargers'] = scaler_target.fit_transform(df[['total_backchargers']])
df['total_grid'] = scaler_target.fit_transform(df[['total_grid']])
df['total_solar'] = scaler_target.fit_transform(df[['total_solar']])
