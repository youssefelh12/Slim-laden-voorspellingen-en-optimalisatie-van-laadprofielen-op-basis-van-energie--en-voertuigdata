...
#     # Reindex to match the hourly consumption dates and fill missing values
...
def add_terugkomdag_feature(df):
    ...
    # Mark an hour as 'terugkomdag' if its date is in the list
...
#     # For hourly data: use a 24h lag and a rolling window of 72 hours (3 days)
#     df['temp_mean_lag_1d'] = df['temperature_2m_mean'].shift(24)
#     df['temp_mean_rolling_3d'] = df['temperature_2m_mean'].rolling(window=72).mean()
...
def add_time_features(df, be_holidays):
    ...
    df['hour_of_day'] = df.index.hour  # New feature for hourly data
...
def add_lag_features(df, be_holidays):
    # For hourly data, lag offsets are multiplied by 24 (hours per day)
    df['consumption_lag_1d'] = df['Total_consumption'].shift(24)
    df['consumption_lag_7d'] = df['Total_consumption'].shift(168)       # 7 * 24
    df['consumption_lag_30d'] = df['Total_consumption'].shift(720)       # 30 * 24
    df['consumption_lag_365d'] = df['Total_consumption'].shift(8760)     # 365 * 24
    df['consumption_lag_14d'] = df['Total_consumption'].shift(336)       # 14 * 24
    df['consumption_lag_21d'] = df['Total_consumption'].shift(504)       # 21 * 24
    df['rolling_avg_3d'] = df['Total_consumption'].rolling(window=72).mean()  # 3 days rolling average
    df['rolling_std_3d'] = df['Total_consumption'].rolling(window=72).std()
...
def plot_correlation_heatmap(df, output_path):
    ...
    plt.title("Hourly Feature Correlation Heatmap", fontsize=16)
    plt.tight_layout()
...
def plot_time_series(df, column, output_path):
    ...
    plt.title('Hourly Power Consumption Over Time', fontsize=14)
...
def plot_forecast_comparison(forecast_df, output_path):
    ...
    plt.title("Actual vs Predicted Hourly Power Consumption", fontsize=14)
    plt.xlabel("Time", fontsize=12)
...
def plot_residuals(forecast_df, output_path):
    ...
    plt.title("Hourly Prediction Residuals (Predicted - Actual)", fontsize=14)
    plt.xlabel("Time", fontsize=12)
...
def plot_error_distribution(forecast_df, output_path):
    ...
    plt.title("Distribution of Hourly Prediction Errors", fontsize=14)
...
def plot_boxplot_comparison(forecast_df):
    ...
    plt.title("Comparison of Real vs Forecasted Hourly Consumption")
...
def fit_sarimax_model_basic(y_train, exog_train_scaled, y_test, exog_test_scaled):
    # Define fixed SARIMAX parameters for hourly data
    order = (1, 1, 1)
    # Seasonal period set to 24 to capture the daily cycle; change to 168 for weekly seasonality if needed
...
# ---------------------------
def main():
    # Define directories and file paths (adjusted for hourly data)
    results_dir = "results/hourly_sarimax"
    create_results_directory(results_dir)
    
    consumption_file = 'api_data/hourly_building_consumption.csv'
    # temp_file = 'api_data/hourly_temperature_data.csv'  # Temperature file (not used)
    
    # Load consumption data and display basic information
    df = load_consumption_data(consumption_file)
    print("Dataset Information (Hourly Data):")
    ...
    # --- Temperature-related steps commented out ---
    ...
    # Add terugkomdag feature (non-temperature)
    ...
    # Exogenous features (temperature-related features commented out)
    exog_features = [
        "dayofmonth", "weekofyear", "day_of_week_sin", "day_of_week_cos", "hour_of_day",
        "is_weekend", "is_festive", "is_summer", "is_winter", 
        # "temperature_2m_mean", "temp_mean_lag_1d", "temp_mean_rolling_3d",  # Temperature features removed
    ...
    print(f"Training data from {df.index[0]} to {df.index[split_index-1]}")
    print(f"Testing data from {df.index[split_index]} to {df.index[-1]}")
    ...
    print("\nAll hourly visualizations have been saved in the 'results/hourly_sarimax' directory.")
    print("Hourly forecasting analysis complete.")
    
    # Boxpblot: Comparison of Real vs Forecasted Values
