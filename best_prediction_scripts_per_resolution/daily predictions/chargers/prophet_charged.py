...
# ---------------------------
# Rolling Forecast CV with Prophet
# ---------------------------
def rolling_forecast_cv_prophet(df, exog_cols, initial_train_size, forecast_horizon=1):
    preds, acts = [], []
    for i in range(initial_train_size, len(df)-forecast_horizon+1):
        tr = df.iloc[:i]
        te = df.iloc[i:i+forecast_horizon]

        # Train frame
        pt = tr.reset_index()[['Day','Total_consumption']].rename(columns={'Day':'ds','Total_consumption':'y'})
        for reg in exog_cols:
            pt[reg] = tr[reg].astype(float).values
        ...
        # Future frame
        pf = te.reset_index()[['Day']].rename(columns={'Day':'ds'})
        for reg in exog_cols:
            pf[reg] = te[reg].astype(float).values

        fc = m.predict(pf)
        preds.extend(fc['yhat'].values)
        acts.extend(te['Total_consumption'].values)
...
def plot_boxplot_comparison(df, path):
    mel=df[['Real_Consumption_kWh','Predicted_Consumption_kWh']].melt(var_name='Type',value_name='Consumption')
    plt.figure(figsize=(8,6)); import seaborn as sns; sns.boxplot(x='Type',y='Consumption',data=mel)
    plt.tight_layout(); plt.savefig(path); plt.close()
...
# ---------------------------
def main():
    res_dir='results/daily_chargers_prophet'
    create_results_directory(res_dir)
    cf='api_data/daily_cleaned_chargers.csv'
    tf='api_data/daily_temperature_data.csv'

    df=load_consumption_data(cf)
    td=load_temperature_data(tf,df.index)
    df=merge_temperature_data(df,td)

    be_h=holidays.BE()
    add_temperature_features(df)
    add_terugkomdag_feature(df)
    add_cumulative_ev_phev_feature(df)
    add_time_features(df,be_h)
    add_lag_features(df,be_h)
    add_holiday_proximity_features(df,be_h)
    add_holiday_type_feature(df,be_h)
    ...
    init_size=int(0.8*len(df))
    acts,preds=rolling_forecast_cv_prophet(df,exogs,init_size)
    ...
    fc_df=pd.DataFrame({'Real_Consumption_kWh':acts,'Predicted_Consumption_kWh':preds},index=acts.index)
    fc_df['Difference']=fc_df['Predicted_Consumption_kWh']-fc_df['Real_Consumption_kWh']
    fc_df['Absolute_Error']=fc_df['Difference'].abs()

    fc_df.to_csv(os.path.join(res_dir,'predicted_values_kwh_cv_prophet.csv'))
    plot_forecast_comparison(fc_df,os.path.join(res_dir,'forecast_comparison.png'))
    plot_residuals(fc_df,os.path.join(res_dir,'residuals.png'))
    plot_error_distribution(fc_df,os.path.join(res_dir,'error_distribution.png'))
    plot_error_by_day(fc_df,os.path.join(res_dir,'error_by_day.png'))
    plot_boxplot_comparison(fc_df,os.path.join(res_dir,'boxplot_comparison.png'))
